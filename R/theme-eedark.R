...
#' }
#'
theme_eedark <- function(size = 10, base_font = "sans", base_color = "#cfd8dc", accent_color = "#717786",
                         faceted = FALSE) {
  half <- size / 2
  ...
  ret <- ret +
    theme(line = element_line(color = base_color, size = .3, linetype = 1),
        ...
        axis.text = ggtext::element_markdown(size = size * .88, color = base_color),
        ...
        plot.title = ggtext::element_markdown(size = 1.7*size, face = "bold", margin = margin(b = .75*size)),
